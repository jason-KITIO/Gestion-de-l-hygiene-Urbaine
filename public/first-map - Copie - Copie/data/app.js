...
fs.writeFile('data.geojson', dataString, (err) => {
  if (err) throw err;
  console.log('Depuis App.js!');
});
...
map.on('load', () => {
    ...
    map.on('click', 'unclustered-point', (e) => {
        ...
        const Ordure =
            e.features[0].properties.Ordure === 1 ? 'yes' : 'no';
        ...
        new mapboxgl.Popup()
            ...
            .setHTML(
                `magnitude: ${mag}<br>Il avait des ordures ?: ${Ordure}`
            )
            .addTo(map);
    });
    ...
    //     // Création d'un objet GeoJSON avec les coordonnées du clic
    //     var features = {
    //         type: 'Features',
    //         geometry: {
    //             type: 'Point',
    ...
    //     // Envoi de l'objet GeoJSON au serveur pour stockage
    //     // var xhr = new XMLHttpRequest();
    //     // xhr.open('POST', 'http://localhost:5000/points.geojson', true);
    //     // xhr.setRequestHeader('Content-Type', 'application/json');
    //     // console.log(features)
    //     // xhr.send(JSON.stringify(features));
